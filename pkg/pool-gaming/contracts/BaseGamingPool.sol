...
import "@balancer-labs/v2-interfaces/contracts/pool-gaming/GamingPoolUserData.sol";
...
/**
 * @dev Base class for GamingPools containing swap, join and exit logic, but leaving storage and management of
 ...
 */
abstract contract BaseGamingPool is BaseMinimalSwapInfoPool {
    using FixedPoint for uint256;
    using GamingPoolUserData for bytes;
    ...
    function _onInitializePool(
        ...
        GamingPoolUserData.JoinKind kind = userData.joinKind();
        _require(kind == GamingPoolUserData.JoinKind.INIT, Errors.UNINITIALIZED);
    ...
    function _doJoin(
        ...
        if (kind == GamingPoolUserData.JoinKind.EXACT_TOKENS_IN_FOR_BPT_OUT) {
            return _joinExactTokensInForBPTOut(balances, normalizedWeights, scalingFactors, totalSupply, userData);
        } else if (kind == GamingPoolUserData.JoinKind.TOKEN_IN_FOR_EXACT_BPT_OUT) {
            return _joinTokenInForExactBPTOut(balances, normalizedWeights, totalSupply, userData);
        } else if (kind == GamingPoolUserData.JoinKind.ALL_TOKENS_IN_FOR_EXACT_BPT_OUT) {
    ...
    function _doExit(
        ...
        GamingPoolUserData.ExitKind kind = userData.exitKind();

        if (kind == GamingPoolUserData.ExitKind.EXACT_BPT_IN_FOR_ONE_TOKEN_OUT) {
            return _exitExactBPTInForTokenOut(balances, normalizedWeights, totalSupply, userData);
        } else if (kind == GamingPoolUserData.ExitKind.EXACT_BPT_IN_FOR_TOKENS_OUT) {
            return _exitExactBPTInForTokensOut(balances, totalSupply, userData);
        } else if (kind == GamingPoolUserData.ExitKind.BPT_IN_FOR_EXACT_TOKENS_OUT) {
